...
const Service = ({serviceInfo}) => {
  ...
  return (
    <div className='border-2 border-c4-0 px-8 py-8 flex flex-col items-center rounded-md space-y-4 justify-between'>
            ...
            <div className='flex space-x-1 justify-around items-center'>
                {
                    serviceInfo.serviceTools.map((serviceTool, serviceToolIndex) => (
                        <div key={serviceToolIndex} className='bg-c4-0 w-full text-c1-0 text-center rounded-sm font-normal text-base px-2 py-[4px]'>
                            <p>{serviceTool}</p>
                        </div>
                    ))
                }
